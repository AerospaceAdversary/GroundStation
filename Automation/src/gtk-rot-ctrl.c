...
static gboolean get_pos(GtkRotCtrl * ctrl, gdouble * az, gdouble * el)
{
    ...
    sat_log_log(SAT_LOG_LEVEL_DEBUG, "get_pos: %s", buffback);
    /* try to parse answer */
    if (retcode)
    {
...
static gboolean set_pos(GtkRotCtrl * ctrl, gdouble az, gdouble el)
{
    ...
    /* send command */
    sat_log_log(SAT_LOG_LEVEL_DEBUG, "set_pos. Az: %d. El: %d.", az, el);
...
static gpointer rotctld_client_thread(GtkRotCtrl* data)
{
...
{
    // gchar          *buff;
    // sat_log_log(SAT_LOG_LEVEL_DEBUG, "gtk_rot_ctrl_update\n");
    ctrl->t = t;
...
void gtk_rot_ctrl_select_sat(GtkRotCtrl * ctrl, gint catnum)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (sat && sat->tle.catnr == catnum)
        {
            ...
            // The following code snippet was copied from sat_selected_cb
            /* update next pass */
            if (ctrl->pass != NULL)
                free_pass(ctrl->pass);
 ...
 */
gboolean rot_ctrl_timeout_cb(GtkRotCtrl* data)
{
    GtkRotCtrl     *ctrl = data;
...
#define SAFE_ELE(ele) CLAMP(ele, ctrl->conf->minel, ctrl->conf->maxel)
     ...
     */
    sat_log_log(SAT_LOG_LEVEL_DEBUG, "Tracking: %d. Target: %d. Az: %lf. El: %lf.", ctrl->tracking, ctrl->target, ctrl->target->az, ctrl->target->el);
    ...
    if (ctrl->tracking && ctrl->target)
    {
        if (ctrl->target->el < 0.0)
        {
            if (ctrl->pass != NULL)
            {
                sat_log_log(SAT_LOG_LEVEL_DEBUG, "Pass. t: %d. pass->aos: %d", ctrl->t, ctrl->pass->aos);
    ...
    sat_log_log(SAT_LOG_LEVEL_DEBUG, "Engaged: %d.", ctrl->engaged);
    if ((ctrl->engaged) && (ctrl->conf != NULL))
    {
            ...
            (fabs(setel - rotel) > ctrl->threshold))
        {
            ...
            sat_log_log(SAT_LOG_LEVEL_DEBUG, "Tracking: %d. Target: %s. Az: %lf. El: %lf. sataz: %lf", ctrl->tracking, ctrl->target->name, ctrl->target->az, ctrl->target->el, setaz);
            if (g_mutex_trylock(&ctrl->client.mutex))
            {
 ...
 */
void rot_locked_cb(GtkRotCtrl* data)
{
    GtkRotCtrl     *ctrl = data;
...
static void gtk_rot_ctrl_init(GtkRotCtrl * ctrl)
{
    ...
    ctrl->tracking = TRUE; // changed from FALSE to TRUE
    ctrl->engaged = FALSE;
    ctrl->delay = 1000;
    ctrl->timerid = 0;
    ctrl->threshold = 5.0;
...
GtkRotCtrl      *gtk_rot_ctrl_new(GtkSatModule * module)
{
    GtkRotCtrl     *rot_ctrl = g_new(GtkRotCtrl, 1);
    gtk_rot_ctrl_init(rot_ctrl);
    // GtkWidget      *table;
    ...
    rot_ctrl->conf = g_try_new(rotor_conf_t, 1);
    rot_ctrl->conf->name = "rotor1";
    if (rotor_conf_read(rot_ctrl->conf))
    {
        sat_log_log(SAT_LOG_LEVEL_INFO,
                    _("Loaded new rotator configuration %s at port %d"),
                    rot_ctrl->conf->name, rot_ctrl->conf->port);
    }
    ...
    rot_ctrl->engaged = 1;
    rot_ctrl->monitor = 0;  // will actually move the rotator
    return rot_ctrl;
}
